// Command ppbundle bundles the paymentproviders TypeScript library into a
// single flat ESM module committed to git and embedded into sercon. Run via
// `make paymentproviders`; `--check` verifies the committed bundle is current.
package main
...
const (
	entry  = "cmd/sercon/paymentproviders/index.ts"
	output = "cmd/sercon/paymentproviders/paymentproviders.bundle.js"
)
...
func main() {
	...
	if *check {
		...
		if err != nil || !bytes.Equal(bytes.TrimSpace(have), bytes.TrimSpace(out)) {
			fmt.Fprintln(os.Stderr, "paymentproviders bundle is stale — run `make paymentproviders` and commit "+output)
			os.Exit(1)
		}
		fmt.Println("paymentproviders bundle up to date")
		return
	}
