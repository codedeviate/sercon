...
// preg2Namespace wires `api.preg2.*` — the PCRE-flavoured sibling of
// `api.preg`. Where `api.preg` runs on Go's RE2 (linear-time, no
// lookaround or backreferences), `api.preg2` runs on
...
//
// The API mirrors `api.preg` exactly (match / matchAll / replace,
