...
func mcpDocs() map[string]scriptengine.MemberDoc {
	return map[string]scriptengine.MemberDoc{
		"serve": {
			...
			Errors:     "Throws synchronously if config is missing/not an object, name/version is missing or empty, or pageSize/handlerTimeout is present but not a valid non-negative integer.",
