...
func cryptoDocs() map[string]scriptengine.MemberDoc {
	return map[string]scriptengine.MemberDoc{
		"hash.md5":      {Summary: "MD5 hex digest of a UTF-8 input. Avoid for security purposes — exposed for compatibility with legacy fingerprints."},
		"hash.sha1":     {Summary: "SHA-1 hex digest of a UTF-8 input. Avoid for security purposes."},
		"hash.sha256":   {Summary: "SHA-256 hex digest of a UTF-8 input."},
		"hash.sha384":   {Summary: "SHA-384 hex digest of a UTF-8 input."},
		"hash.sha512":   {Summary: "SHA-512 hex digest of a UTF-8 input."},
		"hash.sha3_256": {Summary: "SHA-3 256-bit hex digest. The underscore in the name matches recon's binding."},
		"hash.sha3_512": {Summary: "SHA-3 512-bit hex digest."},
		"hash.blake3":   {Summary: "BLAKE3 hex digest (32-byte output, lukechampine.com/blake3)."},
		"hash.crc32":    {Summary: "CRC-32 (IEEE polynomial), zero-padded to 8 hex chars."},
		"jwt.sign":      {Summary: "Sign a claims object. secret is raw bytes for HS*; PEM-encoded private key for RS*/PS*/ES*/EdDSA; or a JWK JSON object (kty picks the key type) for any algorithm. opts.algorithm defaults to HS256."},
		"jwt.view":      {Summary: "Decode header + payload WITHOUT verifying the signature. Useful for inspection / debugging auth flows. Malformed input throws."},
		"jwt.validate":  {Summary: "Verify signature + standard claims (exp/nbf/iat) + optional aud/iss. secret accepts raw bytes / PEM public key / JWK. Set opts.algorithm for the algo-confusion guard. Resolves { valid:true, claims } or { valid:false, reason }."},
		"encrypt.keygen":        {Summary: "Generate a fresh age X25519 keypair. Returns { publicKey: 'age1...', privateKey: 'AGE-SECRET-KEY-1...' }."},
		"encrypt.keygenPgp":     {Summary: "Generate a PGP keypair (RSA 2048). opts.name / opts.email populate the user ID. Returns armored { publicKey, privateKey } blocks. encrypt/decrypt auto-route to PGP when they see these."},
		"encrypt.encrypt":       {Summary: "Seal data to recipients. age public keys (age1...) → age backend (opts.armored for ASCII); PGP public-key blocks → PGP backend (always armored). Auto-dispatched on key format. Multi-recipient: any listed identity decrypts."},
		"encrypt.decrypt":       {Summary: "Open a payload with one of the supplied identities. Routes to age or PGP based on the identity / ciphertext format. age: binary or armored auto-detected. Wrong identity throws."},
		"encrypt.rekey":         {Summary: "Re-encrypt for a new recipient set without exposing plaintext to JS. Output format defaults to match the input; opts.armored forces. Internal decrypt+encrypt loop."},
		"encrypt.detectBackend": {Summary: "Classify a recipient / identity string. Returns { backend: 'age'|'pgp'|'unknown', kind?: 'public'|'private' }. Pure prefix matching; no parsing or I/O."},
	}
}
