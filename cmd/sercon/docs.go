...
func netDocs() map[string]string {
	return map[string]string{
		...
		"capture.openFile":   "Read a .pcap / .pcapng file: net.capture.openFile(path, pkt => {…}) → Promise<void>. Calls the handler once per decoded packet (same shape as capture.open) and resolves at EOF. Offline; no privileges.",
