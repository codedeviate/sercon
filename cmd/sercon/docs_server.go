...
await srv.close();`,
		...
		"icmp.listen": {
			...
			Params: []scriptengine.Param{
				{Name: "opts", Type: "{ network?: \"ip4\" | \"ip6\" }", Optional: true, Desc: "network selects the IP version (default 'ip4'). There is no host/port — raw ICMP binds to all addresses."},
				{Name: "handler", Type: "(msg: { bytes: Uint8Array; text: string; address: string; type: number; code: number }, reply: (opts?: { to?: string; type?: number; code?: number; id?: number; seq?: number; payload?: string | Uint8Array; body?: string | Uint8Array }) => Promise<void>) => void", Optional: true, Desc: "Invoked once per received ICMP packet. msg carries the marshalled body (bytes/text), the sender address, and the parsed type/code. reply(opts?) sends an ICMP message back to the sender (or opts.to): Echo mode { type?, code?, id?, seq?, payload? } or raw mode { type, code?, body } (body marshalled verbatim); it returns a Promise that resolves once written."},
			},
			ReturnType: "{ address: string; close(): Promise<void> }",
