...
// sqlHandleType is the TS type every db.<sql-engine>.open resolves to. The
// six SQL engines (sqlite/postgres/mysql/mssql/clickhouse/oracle) share one
// handle shape (see sqlHandle in db_sql.go); only the placeholder syntax
// differs, which is documented per-engine in Returns. Defined once so the
// emitted d.ts + reference stay identical across engines.
const sqlHandleType = "Promise<{ exec(sql: string, ...params: unknown[]): Promise<{ rowsAffected: number, lastInsertId: number }>; query(sql: string, ...params: unknown[]): Promise<Record<string, unknown>[]>; queryValue(sql: string, ...params: unknown[]): Promise<unknown>; begin(): Promise<{ exec(sql: string, ...params: unknown[]): Promise<{ rowsAffected: number, lastInsertId: number }>; query(sql: string, ...params: unknown[]): Promise<Record<string, unknown>[]>; queryValue(sql: string, ...params: unknown[]): Promise<unknown>; commit(): Promise<void>; rollback(): Promise<void> }>; prepare(sql: string): Promise<{ exec(...params: unknown[]): Promise<{ rowsAffected: number, lastInsertId: number }>; query(...params: unknown[]): Promise<Record<string, unknown>[]>; queryValue(...params: unknown[]): Promise<unknown>; close(): Promise<void> }>; close(): Promise<void> }>"

// respHandleType is the TS type db.redis.open / db.valkey.open resolve to —
// the RESP { do, ping, close } handle (see respHandle in redis.go).
const respHandleType = "Promise<{ do(cmd: string, ...args: unknown[]): Promise<unknown>; ping(): Promise<string>; close(): Promise<void> }>"
...
func dbDocs() map[string]scriptengine.MemberDoc {
	return map[string]scriptengine.MemberDoc{
		"sqlite.open": {
			...
			Example: `const conn = await db.sqlite.open(":memory:");
...
await r.close();`,
		...
		"memcached.open": {
			...
			ReturnType: "Promise<{ get(key: string): Promise<string | null>; set(key: string, value: unknown, expirySeconds?: number): Promise<void>; delete(key: string): Promise<boolean> }>",
...
const existed = await mc.delete("session:42"); // true`,
		...
		"ldap.open": {
			...
			ReturnType: "Promise<{ rootDSE(): Promise<Record<string, unknown>>; search(baseDN: string, filter?: string, attrs?: string[]): Promise<Record<string, unknown>[]>; close(): Promise<void> }>",
...
await dir.close();`,
		...
		"dict.define": {
			...
			ReturnType: "Promise<{ word: string, found: boolean, definitions: { db: string, dbName: string, text: string }[] }>",
...
if (r.found) runtime.log(r.definitions[0].text);`,
		...
		"dict.match": {
			...
			ReturnType: "Promise<{ word: string, matches: { db: string, word: string }[] }>",
