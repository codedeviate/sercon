...
// { note: { "@id": "5", "#text": "hi" } }`,
		...
		"sheet.write": {
			...
			Params: []scriptengine.Param{
				...
				{Name: "opts", Type: "{ format: \"csv\" | \"tsv\" | \"xlsx\" | \"ods\"; dest?: string }", Desc: "format selects the writer (or is inferred from a dest extension); dest, when set, writes the file and returns its path instead of bytes."},
			},
			...
			Errors:     "Throws if the model isn't an object-with-sheets or a 2D array, if format is missing/unsupported, if a CSV/TSV write has more than one sheet, if the format is read-only (xls/slk/dif throws \"<fmt> is read-only (extract/convert only)\"), or on an encode/write failure.",
			Example:    "const out = codec.sheet.write([[\"a\", 1, true]], { format: \"ods\" });\nruntime.log(out.format, out.bytes.length);",
		},
		"sheet.formats": {
			Summary:    "Report the read/write capability of every spreadsheet format codec.sheet supports. Read-only formats (xls/slk/dif) have write:false.",
			Params:     []scriptengine.Param{},
			...
			Example:    `const f = codec.sheet.formats(); if (!f.xls.write) console.log("xls is read-only");`,
		},
		"toml.parse": {
