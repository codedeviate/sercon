...
// { note: { "@id": "5", "#text": "hi" } }`,
		...
		"sheet.read": {
			...
			Params: []scriptengine.Param{
				{Name: "src", Type: "string | Uint8Array", Desc: "A file path or the encoded bytes (csv/tsv/xlsx/ods)."},
				{Name: "opts", Type: "{ format?: \"csv\" | \"tsv\" | \"xlsx\" | \"ods\" }", Optional: true, Desc: "Override the auto-detected format (e.g. force tsv for tab-delimited bytes)."},
			},
