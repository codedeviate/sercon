...
- `cmd/sercon/main.go` — CLI plus the eleven reserved top-level globals (`runtime`, `crypto`, `text`, `codec`, `fs`, `net`, `db`, `server`, `services`, `tui`, `image`), each registered as a namespace factory by `registerSurface`.
...
- `MANUAL.md` — long-form reference; covers the library API, CLI, the eleven reserved script globals, the `server` namespace (§6), goja built-ins, eventloop additions.
...
- `examples/scripts/sercon.d.ts` — auto-generated TypeScript declaration file mirroring the CLI's reserved-global surface (`runtime`, `crypto`, `text`, `codec`, `fs`, `net`, `db`, `services`, `tui`, `image`). Regenerate via `make types` whenever the CLI binding set or the d.ts emitter changes. Tracked in git so editor autocomplete and PR reviewers see the surface without running the binary.
- `MANUAL.md § 16. Binding reference` — a GENERATED section between `<!-- BEGIN/END GENERATED REFERENCE -->` markers. Do NOT hand-edit it; run `make reference` (builds sercon, runs `--emit-reference`, splices via awk — idempotent). `make manual` runs `make reference` first. The reference is generated from the structured `scriptengine.MemberDoc` docs in `cmd/sercon/docs.go`; richer per-function docs come from filling each member's `Params`/`ReturnType`/`Returns`/`Errors`/`Example` (which also upgrades the d.ts signature + `@param`). All eleven reserved-global namespaces + `console` are migrated to the structured model; keep new bindings structured (fill Params/ReturnType/Returns/Errors/Example) so the d.ts + reference stay complete.
...
- **`.github/workflows/release.yml`** fires on `v*.*.*` tag push (cut manually by the maintainer via `make release-prep` + `git tag` + `git push`). Calls goreleaser with `.goreleaser.yml`, which cross-compiles darwin-{amd64,arm64} / linux-{amd64,arm64} / windows-amd64, mirrors `make release`'s `-trimpath -ldflags='-s -w'`, and uploads tarballs/zip + checksums to the auto-created GitHub Release. Each archive bundles LICENSE, README, CHANGELOG, MANUAL.md, and MANUAL.pdf. A second `homebrew` job then bumps the Homebrew formula: it checks out the `codedeviate/homebrew-cli` tap and runs that tap's `scripts/bump.sh sercon <version>` (recomputes the source-tarball sha256 and rewrites `Formula/sercon.rb`), then commits the result. It needs a `HOMEBREW_TAP_GITHUB_TOKEN` repository secret — a fine-grained PAT with Contents:write scoped to `homebrew-cli` — and skips cleanly (release unaffected) if the secret is absent. The formula stays a source formula (`brew install codedeviate/cli/sercon`, macOS + Linux); we did not switch to goreleaser/casks. Auto-bump starts with the next release; 0.35.1 was the last hand-bumped version.
