...
- `MANUAL.md § 16. Binding reference` — a GENERATED section between `<!-- BEGIN/END GENERATED REFERENCE -->` markers. Do NOT hand-edit it; run `make reference` (builds sercon, runs `--emit-reference`, splices via awk — idempotent). `make manual` runs `make reference` first. The reference is generated from the structured `scriptengine.MemberDoc` docs in `cmd/sercon/docs.go`; richer per-function docs come from filling each member's `Params`/`ReturnType`/`Returns`/`Errors`/`Example` (which also upgrades the d.ts signature + `@param`). All ten reserved-global namespaces + `console` are migrated to the structured model; keep new bindings structured (fill Params/ReturnType/Returns/Errors/Example) so the d.ts + reference stay complete.
