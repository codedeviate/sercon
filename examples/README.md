...
| `mcp-client-http.ts` | `mcp.connect.http` — a hermetic server+client round-trip: starts an in-script MCP server (`mcp.serve` + `srv.listen({port:0})`) and connects to it with `mcp.connect.http`, exercising the full Phase-1 consume surface (`serverInfo`, `listTools`, `callTool` incl. an `isError` case, `listResources`/`readResource`, `listPrompts`/`getPrompt`, `ping`) before closing both. Fully offline and self-contained. |
