...
declare const api: {
  ...
  /** Script-host scaffolding: logging, assertions, time, environment. */
  runtime: {
    assert: {
  ...
  /** Subprocess and external-CLI wrappers: shell, git, gh, AI providers. */
  tools: {
    ai: {
  ...
  /** Interactive terminal surface: multi-pane TUI. */
  ui: {
    /** Multi-pane TUI runtime. layout(tree) declares panes (rows/cols recursive tree); pane(name) returns a handle with write/writeln/clear/title. api.tools.exec.shell({pane}) streams subprocess I/O into a pane. Auto-falls-back to prefixed plain-text lines when stdout is not a TTY. */
    tui: {
