...
api.log("argv:", JSON.stringify(Sercon.argv));
api.log("program:", Sercon.argv[0]);
api.log("script:", Sercon.argv[1]);

const userArgs = Sercon.argv.slice(2);
api.log("user args:", JSON.stringify(userArgs));

// argv always holds at least [programName, scriptPath], even with no `--`
// args — so a script can read it unconditionally.
api.assert.ok(Sercon.argv.length >= 2, "argv should hold program + script");
api.assert.ok(Sercon.argv[1].endsWith("argv.ts"), "argv[1] is the running script");
