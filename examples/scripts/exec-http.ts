...
// email-auth.ts). The httpbin.org section self-skips (exit 0) when the
// endpoint is unreachable; genuine errors are re-thrown.
...
try {
  ...
  runtime.log("httpbin.org unreachable — skipping exec.http demo. (" + String(e).slice(0, 120) + ")");
}

// Intentional transport error against a local closed port — deterministic
// (connection refused regardless of internet), so it runs unconditionally.
runtime.log("");
