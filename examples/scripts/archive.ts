// Demonstrates api.archive.* — create + extract over zip / tar / tar.gz.
// Uses a few temporary files under the host's temp dir so the script
// cleans up after itself.

const tmp = api.env.get("TMPDIR") ?? "/tmp";
const work = api.path.dirname(tmp + "/sercon-archive-demo");
...
for (const ext of [".zip", ".tar", ".tar.gz"]) {
  ...
  const out = await api.archive.create(dest, sources);
  api.log(`  ${ext.padEnd(7)} ${out.bytes.toString().padStart(6)} bytes  entries=${out.entries.length}  -> ${dest}`);

  // Extract into a sibling directory and list what came back.
  const dir = tmp + "/sercon-demo-extracted-" + ext.replace(/[^a-z]/g, "");
  const got = await api.archive.extract(dest, dir, { overwrite: true });
  api.log(`         extracted ${got.entries.length} entries  -> ${dir}`);
}

api.log("");
api.log("Tip: api.archive.create also takes [{path, name}] objects to override the in-archive name.");
